...
export async function generateTeamProfilePdfBefore(
  data: TeamPdfData,
  ...
  if (sections.leadership && Array.isArray(s.leadership) && s.leadership.length > 0) {
    ...
    for (let i = 0; i < Math.min(3, s.leadership.length); i++) {
      ...
      doc.setFontSize(11);
      doc.setTextColor(...NAVY);
      doc.text(`${i + 1}. ${cleanMarkdown(it.headline ?? "")}`, MARGIN_L, ctx.y);
      ctx.y += 5;
      ...
      if (it.action) {
        ...
        doc.setFontSize(9);
        doc.setTextColor(...NAVY);
        doc.text(cleanMarkdown(it.action), MARGIN_L, ctx.y);
        ctx.y += 5;
      }
