...
export async function renderCoverPage(doc: jsPDF, p: CoverParams): Promise<void> {
  ...
  const pillText = p.contextPill.toUpperCase();
  doc.setFont("Montserrat", "semibold");
  doc.setFontSize(9);
  ...
  doc.setTextColor(...ORANGE);
  doc.setCharSpace(1);
  doc.text(p.participantLabel.toUpperCase(), MARGIN_L, fieldY);
  doc.text(p.dateLabel.toUpperCase(), MARGIN_L + fieldColW + 10, fieldY);
  doc.setCharSpace(0);

  doc.setFont("Poppins", "bold");
  doc.setFontSize(15);
  doc.setTextColor(...NAVY);
  doc.text(p.participantValue, MARGIN_L, fieldY + 9);
  doc.text(p.dateValue, MARGIN_L + fieldColW + 10, fieldY + 9);
  ...
  const disclaimerY = fieldY + 28;
  const disclaimerText =
    "This report is generated by an AI model and is intended for personal reflection only. It does not constitute a clinical diagnosis or professional psychological advice. Results should be interpreted in conjunction with qualified professional guidance.";
  doc.setFont("Montserrat", "normal");
  doc.setFontSize(9);
  ...
  const copyText = `© ${new Date().getFullYear()} BrainWise Enterprises. Confidential and proprietary. Shared with the named recipient for evaluation only; not to be reproduced or disclosed without written consent.`;
