...
export default function ServiceDetailModal({ card, onClose, onOpenBriefing }: Props) {
  const titleId = useId();
  ...
  const handleCtaClick = () => {
    if (card.cta.action === "open-briefing") {
      onClose();
      // Defer slightly so close completes before briefing opens
      setTimeout(() => onOpenBriefing(), 0);
    }
  };

  const ctaButton =
    card.cta.action === "navigate" && card.cta.to ? (
    ...
    <div className="bw-marketing-root">
      ...
      >
        ...
        >
          ...
          <div style={eyebrowStyle}>Service Detail</div>
