...
export const NewsletterStepList = Node.create({
  ...
  // §151 (H5 Cycle 2): no import-fallback rule. Parent "newsletterStep+" plus child "heading block*" requirement (heading-first) creates a two-layer coercion failure. External markup cannot satisfy the schema's content expression and ProseMirror's content coercion would drop the wrapper silently.
  parseHTML() {
    return [
      {
