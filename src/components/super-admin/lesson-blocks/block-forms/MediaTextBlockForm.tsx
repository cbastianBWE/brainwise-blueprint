...
interface Props {
  value: MediaTextBlockValue;
  onConfigChange: (next: MediaTextBlockValue) => void;
  contentItemId?: string;
}

export function MediaTextBlockForm({ value, onConfigChange, contentItemId }: Props) {
    ...
    <div className="space-y-4">
      <div className="space-y-2">
        ...
        <FileUploadField
          ...
          onChange={(newAssetId) => onConfigChange({ ...value, asset_id: newAssetId })}
        />
      </div>
      ...
      <Collapsible open={pexelsOpen} onOpenChange={setPexelsOpen}>
        ...
        <CollapsibleContent className="space-y-3 pt-3">
          {!canSearchPexels && (
            <p className="text-xs text-muted-foreground">Save the lesson first to use Pexels.</p>
          )}
      ...
      <div className="space-y-2">
        ...
        <Input
          value={value.caption ?? ""}
          onChange={(e) => onConfigChange({ ...value, caption: e.target.value || null })}
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
        <div className="space-y-2">
          ...
          <Select
            value={value.media_position ?? "left"}
            onValueChange={(v) =>
              onConfigChange({ ...value, media_position: v as "left" | "right" })
            }
          >
        ...
        <div className="space-y-2">
          ...
          <Select
            value={value.media_ratio ?? "half"}
            onValueChange={(v) =>
              onConfigChange({ ...value, media_ratio: v as "half" | "third" })
            }
          >
            <SelectTrigger><SelectValue /></SelectTrigger>
            <SelectContent>
              <SelectItem value="half">Equal</SelectItem>
              <SelectItem value="third">Image narrower</SelectItem>
            </SelectContent>
          </Select>
        </div>
        <div className="space-y-2">
          ...
          <Select
            ...
            onValueChange={(v) =>
              onConfigChange({ ...value, vertical_align: v as "top" | "center" })
            }
          >
            <SelectTrigger><SelectValue /></SelectTrigger>
            <SelectContent>
