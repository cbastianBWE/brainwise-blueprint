...
export default function QuestionRendererMatchPicture({
  prompts,
  answers,
  value,
  onAnswer,
  ...
  const answerImageUrl = (a: MatchOption): string | undefined =>
    a.option_image_asset_id ? imageUrlMap?.get(a.option_image_asset_id) : undefined;

  return (
    <div className="space-y-3">
      <p className="text-sm text-muted-foreground">
        Tap a prompt on the left, then tap its matching picture on the right. Tap again to unpair.
      </p>
          ...
          {answers.map((a) => {
            ...
            return (
              <button
                ...
                className={cn(
                  "w-full rounded-lg border-2 bg-card px-4 py-3 transition-colors",
              ...
              >
                {imgUrl ? (
                  <img
                    src={imgUrl}
                    alt={a.option_text}
                    className="mx-auto max-h-32 w-auto rounded-md object-contain"
                  />
                ) : (
