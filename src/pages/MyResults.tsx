  ...
  useEffect(() => {
    ...
    const poll = async () => {
      const { data } = await supabase
        ...
        .select("narrative_status, ai_narrative, facet_insights_all_total")
      ...
      const raw = row?.narrative_status ?? null;
      const rawStatus: PtpNarrativeStatus =
      ...
      else if ((rawStatus === "generating" || rawStatus === "pending") && contentReady) next = "ready";
...
export default function MyResults({ isCoachView = false, adminView = false, targetUserId, preSelectedAssessmentId, coachUserId, permissionLevel = null, viewLabel, defaultInstrumentId, allowHighlighting = true }: MyResultsProps) {
  ...
  const [dimensionFacetRanges, setDimensionFacetRanges] = useState<Record<string, { low: number; high: number }>>({});
