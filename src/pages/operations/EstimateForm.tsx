...
export default function EstimateForm() {
  ...
  const estimateQ = useQuery({
    queryKey: ["ops", "estimate", id, "edit-prefill"],
    enabled: isEdit,
    queryFn: async () => {
      const { data, error } = await opsSupabase
        .from("estimates")
  ...
  const linesQ = useQuery({
    queryKey: ["ops", "estimate-lines", id, "edit-prefill"],
    enabled: isEdit,
    queryFn: async () => {
      const { data, error } = await opsSupabase
        .from("document_lines")
  ...
  useEffect(() => {
    ...
    if (!estimateQ.data || !linesQ.data) return;
    const est: any = estimateQ.data;
    setHeader({
      customer_id: est.customer_id ?? "",
      issue_date: est.issue_date ?? todayISO(),
      expiration_date: est.expiration_date ?? "",
      currency_code: est.currency_code ?? "USD",
      reference_number: est.reference_number ?? "",
      discount_amount: est.discount_amount != null ? String(est.discount_amount) : "0",
      adjustment_amount: est.adjustment_amount != null ? String(est.adjustment_amount) : "0",
    ...
    setPrefilled(true);
  }, [isEdit, prefilled, estimateQ.data, linesQ.data]);
  ...
  const total = useMemo(
    ...
    [subtotal, header.discount_amount, header.adjustment_amount],
  );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isEdit) {
        const { error } = await supabase.rpc("ops_update_estimate" as any, {
          p_id: id,
          p_header,
          p_lines,
        });
        ...
        navigate(`/operations/estimates/${id}`);
      } else {
        const { data, error } = await supabase.rpc("ops_create_estimate" as any, {
          p_header,
          p_lines,
        });
        if (error) throw error;
        ...
        navigate(`/operations/estimates/${newId}`);
      }
    } catch (err: any) {
  ...
  if (isEdit && (estimateQ.isLoading || linesQ.isLoading)) {
  ...
  return (
    <div className="p-6 space-y-6">
      <div>
        <h1 className="text-2xl font-semibold">{isEdit ? "Edit estimate" : "New estimate"}</h1>
        <p className="text-muted-foreground text-sm">Operations · Estimate</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e) => setH("expiration_date", e.target.value)}
                />
              </div>
              <div className="space-y-2">
        ...
        <div className="flex justify-end gap-2">
          <Button
            type="button"
            variant="outline"
            onClick={() => navigate(isEdit ? `/operations/estimates/${id}` : "/operations/estimates")}
            disabled={submitting}
          >
            Cancel
          </Button>
...
function invalidateAll(qc: ReturnType<typeof useQueryClient>, estimateId: string, customerId: string) {
  qc.invalidateQueries({ queryKey: ["ops", "estimates", "list"] });
  qc.invalidateQueries({ queryKey: ["ops", "estimate", estimateId] });
  if (customerId) {
    qc.invalidateQueries({ queryKey: ["ops", "customer-estimates", customerId] });
  }
}
